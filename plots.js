var apiKey = "QsSzTqfa-CSUBMhqzF8D";
...
function getMonthlyData() {
  var apiKey = "QsSzTqfa-CSUBMhqzF8D";
  //var ticker = d3.select("#stockInput").node().value;
  ...
  d3.json(queryUrl).then(function(data) {
    ...
    // @TODO: Unpack the dates, open, high, low, close, and volume
        ...
        //console.log(dates, openPrices, highPrices, lowPrices, closingPrices, volume)
...
function buildTable(dates, openPrices, highPrices, lowPrices, closingPrices, volume) {
  ...
  var table = d3.select("#high-low");
  var tbody = table.select("tbody");
  var trow;

  trow = tbody.append("tr");
  trow.append("td").text(Math.max(...highPrices));
  trow.append("td").text(Math.min(...lowPrices));
...
function buildPlot() {
  ...
  d3.json(url).then(function(data) {
    ...
    console.log(Math.max(...highPrices))
    getMonthlyData();
